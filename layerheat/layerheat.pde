...
void setup() {
   ...
   // ysize = cpus.size() + 4; 
   ysize = ( cpus.size() > maxItemsperLayer ? maxItemsperLayer : cpus.size()) + 4;
...
void newdrawing() {  
   String[] info;    
     ...
     final JFileChooser fc = new JFileChooser("/Users/exsanche/processing/layerheat/data"); 
...
void draw() {
  ...
  fill(0,0,0);
  textFont(font,12);
  text("Disks Stats for "+servername+" on "+datestr,XaxisOffset+(xsize/4)*GridSize, Yheader * 2); 
  text("(move the mouse over grid to display disk information and usage)",XaxisOffset+(xsize/4)*GridSize-30, Yheader * 2+14); 
  drawKeyChart();

  String[] s = new String[maxItemsperLayer];
  String cpu_label;
  int maxItemsperLoop = maxItemsperLayer;
  ...
  */
  ...
  for ( int i = 0; i < maxItemsperLayer; i++) {
      if ( ( maxItemsperLayer*layer+i ) < cpus_list.size() ) {
          s[i] = (String) cpus_list.get(i+(layer*maxItemsperLayer));
      } else {
          maxItemsperLoop = i;
          break;
      }
  ...
  colorMode(RGB, 255);
  for (int cpu_index = 0; cpu_index < maxItemsperLoop; cpu_index++) {
  ...
  fill(255,255,255);   
  rect( XaxisOffset/2, YaxisOffset,GridSize * 4, ( (maxItemsperLayer+5)*GridSize));
  fill(0,0,0);
  ...
  if ( inDataBoxArea(x,y,DataBoxSize) ) {
      ...
      if ( y == yo && x == xo ) {
          ...
          if ( x > 0 && x < sample_times.size()*GridSize  && y > 0 && y < maxItemsperLoop*GridSize) {
...
boolean inDataBoxArea (int x, int y, int DataBoxSize) {
    if ( (x >= (xsize+offset)*GridSize) && (x <= (xsize+DataBoxSize+offset)*GridSize) && (y < maxItemsperLayer*GridSize+fontSize*3)  ) {
        return true;
    } else {
        return false;
    }
...
void drawDataBox (int DataBoxSize) {
    fill(255,255,255);
    rect( XaxisOffset+(xsize+offset)*GridSize, YaxisOffset, GridSize*DataBoxSize, (maxItemsperLayer)*GridSize+fontSize*3);
}

void drawScrollBar( int layer, int DataBoxSize ) {
      fill(255,255,255);
      stroke(0);
      rect( XaxisOffset+(xsize+offset+DataBoxSize/2-1)*GridSize, YaxisOffset, 2*GridSize, maxItemsperLayer*GridSize+fontSize*3);
...
void mouseDragged() {
  int maxLayers;
    ...
    maxLayers = cpus.size() / maxItemsperLayer;
    if ( follow == 1 ) {
...
void loadfile(String servername, String iostatfile ) {
    ...
    try {       
        String timespec = "[0-2][0-9]:[0-5].*";
        String itemspec = "hdiskpower[0-9]*";
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if ( current_line.length > 3 && (current_line[0].matches(timespec) || current_line[0].matches(itemspec) ) )    {
                if ( current_line[0].matches(timespec)) {
                ...
                if ( cpu_label.matches(itemspec) ) {
                    if (  cpus.get(cpu_label) == null ) {
                        // This is for a new device, so let's create a whole new array of data points
                ...
                } // if-then-else  cpu_label.matches(itemspec)
            }
        } // for i... 
    } catch (Exception ignore) {
         print("error \n");
